...
import Haxxor from "../Boss/Special/Haxxor";
...
export default class AiTank extends TankBody {
    ...
    public constructor(game: GameServer,owner: ObjectEntity , megaTanks = false) {
        ...
        this.nameData.values.name = "A1 T4NK"
        ...
        if(!megaTanks){
            ...
            for (let i = Stat.MovementSpeed; i < Stat.HealthRegen; ++i)  this.camera.cameraData.values.statLevels.values[i] = 2;
            ...
            for (let i = Stat.MovementSpeed; i < Stat.HealthRegen; ++i)  this.camera.cameraData.values.statLevels.values[i] = 3;
            this.camera.cameraData.values.statLevels.values[Stat.MovementSpeed] = 2;
    ...
    public destroy(animate=true) {
        if (!animate) {
            if(this.owner instanceof Haxxor) this.owner.currentTankAmount -= 1;
        }
