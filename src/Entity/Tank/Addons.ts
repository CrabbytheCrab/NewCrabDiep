...
/** The thing above shotguns's and dual-barrel's barrels. */
class ShotPronouncedAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        const pronounce = new ObjectEntity(this.game);
        const sizeRatio = 75 / 50;
        const widthRatio = 42 * 1.19048 / 50;
        const offsetRatio = 1 /20;
        const size = this.owner.physicsData.values.size;
...
/** The thing above pelletshot's barrels. */
class PelletPronouncedAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);
        ...
        pronounce.physicsData.values.size = sizeRatio * size;
        pronounce.physicsData.values.width = widthRatio * size;
        pronounce.positionData.values.x = pronounce.physicsData.values.size / 2;
...
/** The thing underneath Pelletshot addon. */
class PelletLauncherAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        const launcher = new ObjectEntity(this.game);
        const sizeRatio = 95 / 50;
        const widthRatio = 42 * 0.809524 / 50;
        const size = this.owner.physicsData.values.size;
...
class GliderLauncherAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);
...
//Firework's values are a bit off mainly due to the trapezoid ratio being 3:4 for some ungodly reason instead of 4:7 and diep custom doesn't support custom ratios for trapezoids

/** The thing above firework's barrel. */
class FireworkPronouncedAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        const pronounce = new ObjectEntity(this.game);
        const sizeRatio = 45 / 50;
        const widthRatio = 53.55 / 50;
        const offsetRatio = 3 / 10;
        const size = this.owner.physicsData.values.size;
...
/** The thing underneath Firework addon. */
class FireworkLauncherAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        const launcher = new ObjectEntity(this.game);
        const sizeRatio = 80 / 50;
        const widthRatio = 29.4 / 50;
        const offsetRatio = 3 / 10;
        const size = this.owner.physicsData.values.size;
        ...
        launcher.physicsData.values.size = sizeRatio * size;
        launcher.physicsData.values.width = widthRatio * size;
        launcher.positionData.values.x = (launcher.physicsData.values.size / 2) + (offsetRatio * size);
...
export const AddonById: Record<addonId, typeof Addon | null> = {
    ...
    // a part of post 2018 diep
    shotpronounced: ShotPronouncedAddon,
    pelletpronounced: PelletPronouncedAddon,
    pelletlauncher: PelletLauncherAddon,
    gliderlauncher: GliderLauncherAddon,
    fireworkpronounced: FireworkPronouncedAddon,
    fireworklauncher: FireworkLauncherAddon,
    // not part of diep
